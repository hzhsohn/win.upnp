...
bool CzhUPnP::InternalSearch(int version)
{
...
bool CzhUPnP::Search(int version)
{
...
CString CzhUPnP::GetProperty(const CString& name, CString& response)
{
...
bool CzhUPnP::InvokeCommand(const CString& name, const CString& args)
{
...
bool CzhUPnP::addPortmap(int eport, int iport, const CString& iclient, const CString& descri, const CString& type)
{
	CString args;
...
bool CzhUPnP::deletePortmap(int eport, const CString& type)
{
	CString args;
...
/////////////////////////////////////////////////////////////////////////////////
CzhUPnP::UPNPNAT_RETURN CzhUPnP::AddNATPortMapping(UPNPNAT_MAPPING *mapping, bool tryRandom)
{
	...
	for (int retries = 255; retries; retries--) {
		CString Desc;
		Desc.Format(_T("%s [%s: %u]"), mapping->description, ProtoStr, mapping->externalPort);
...
//			Error, use GetLastError() to get an error description.
/////////////////////////////////////////////////////////////////////////////////
CzhUPnP::UPNPNAT_RETURN CzhUPnP::RemoveNATPortMapping(UPNPNAT_MAPPING mapping, bool removeFromList)
{
	if(!IsLANIP(GetLocalIP())){
		SetLastError(_T("You aren't behind a Hardware Firewall or Router"));
		return UNAT_NOT_IN_LAN;
...
void CzhUPnP::clearNATPortMapping()
{
	UPNPNAT_MAPPING search;
	POSITION pos = m_Mappings.GetHeadPosition();
	while(pos){
		...
		char szHost[256];
		 ...
		 { 
			if (gethostname(szHost, sizeof(szHost)) == 0){
			 ...
			 //WSACleanup( );这句加了就会出现不能找到UPNP设备的错误
		 }
	 }
#ifndef _DEBUG
	catch(...){
...
/////////////////////////////////////////////////////////////////////////////////
void CzhUPnP::SetLastError(CString error)	{
	m_slastError = error;
};
